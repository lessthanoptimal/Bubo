...
public class OccupancyGridIO {
    ...
    /**
     * Converts an image into a ArrayGrid2D_I8
     *
     * @param image Input image which is being converted
     * @return The new map.
     */
    public static ArrayGrid2D_I8 load_I8( BufferedImage image ) {
        int width = image.getWidth();
        int height = image.getHeight();

        ArrayGrid2D_I8 map = new ArrayGrid2D_I8(width,height);
    ...
    /**
     * Converts OccupancyGrid2D_I into an image
     *
     * @param map Occupancy grid
     * @return An image of the map.
     */
    public static BufferedImage render_I( OccupancyGrid2D_I map ) {
        int width = map.getWidth();
        int height = map.getHeight();
        ...
        for( int y = 0; y < height; y++ ) {
            for( int x = 0; x < width; x++ ) {
                int val = (map.get(x,y) & 0xFF)*255/max;
                int rgb = val << 16 | val << 8 | val;
